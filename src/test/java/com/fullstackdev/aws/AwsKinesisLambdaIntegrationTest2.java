...
@Testcontainers
public class AwsKinesisLambdaIntegrationTest2 {
	
	private static final Logger logger =  LoggerFactory.getLogger(AwsKinesisLambdaIntegrationTest2.class);
	...
	@Container
	  static LocalStackContainer localStack = new LocalStackContainer(DockerImageName.parse("localstack/localstack"))
			  																				...
			  																				.withCopyFileToContainer(MountableFile.forHostPath(new File("C:\\Users\\deepika\\eclipse-workspace2\\localstack-kinesis-s3-demo\\target\\localstack-kinesis-s3-demo.jar").getPath()), "/tmp/localstack/localstack-kinesis-s3-demo.jar")
	...
	public  S3Client s3Client(LocalStackContainer localStackContainer) throws URISyntaxException {
		logger.info("S3 service :",localStackContainer.getEndpointConfiguration(LocalStackContainer.Service.S3).getServiceEndpoint());
		...
		try {
			...
			endpointOverride = new URI("http://" + localStackContainer.getContainerIpAddress() + ":4566");
	...
	public void testContainer() throws UnsupportedOperationException, IOException, InterruptedException, URISyntaxException {
		...
		S3Client s3Clientobj = s3Client(localStack);
		
		logger.info("buckets :",s3Clientobj.listBuckets());
